...
class TestFileEndpoints(BaseTestCase):
    ...
    @patch("storage.connectors.google_drive.GoogleDriveConnector.stream_file")
    def test_download_file(self, mock_stream):
        """Test signed download token flow streams content securely."""
        content = b"Hello, World!!"
        mock_stream.return_value = (iter([content]), "text/plain")

        # Step 1: Get a signed download token (authenticated).
        token_response = self.client.get(
            reverse("file-download-token", kwargs={"pk": self.file.id})
        )
        self.assertEqual(token_response.status_code, 200)
        download_url = token_response.data["download_url"]
        self.assertIn("token=", download_url)

        # Step 2: Download using the signed token (no auth header needed).
        from urllib.parse import parse_qs, urlparse

        parsed = urlparse(download_url)
        token = parse_qs(parsed.query)["token"][0]

        # Clear auth so we prove the token alone is sufficient.
        ...
        # Verify a tampered token is rejected.
